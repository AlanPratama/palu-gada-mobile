...
export default function BottomSheetPostDeleteAlert({
  refRBSheet,
  post,
  resetPostItems,
}) {
...
const PostDeleteAlertComp = ({ refRBSheet, post, resetPostItems }) => {
  ...
  const onSubmit = async () => {
    ...
    if (res.data.status === "OK") {
      ToastAndroid.show("Berhasil Menghapus Post!", 1500);
      refRBSheet.current?.close();
      resetPostItems();
      navigate.goBack();
    } else {
      ToastAndroid.show("Gagal Menghapus Post!", 1500);
    }
  };
      ...
      >
        <View style={{ marginTop: 26 }} className="px-8">
          <Text className="text-base font-semibold text-[#343434] mb-2 text-center">
            Apakah kamu yakin akan menghapus postingan '{post.title}'?
          </Text>
